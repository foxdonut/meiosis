...
exports.createMithrilRouter = exports.RouteChangeEffect = exports.createRouter = exports.getQueryString = void 0;
// ----- Helpers
...
const separateParamsAndQueryParams = (path, allParams) => {
    ...
    return Object.entries(allParams).reduce((result, [key, value]) => {
...
const createGetUrl = (prefix, historyMode, wdw) => historyMode
...
const ToUrl = (routeConfig, getStatePath, queryString) => {
    const pathLookup = Object.entries(routeConfig).reduce((result, [path, page]) => Object.assign(result, { [page]: path }), {});
    return (page, allParams = {}) => {
        ...
        return ((path.match(/(:[^/]*)/g) || []).reduce((result, pathParam) => result.replace(new RegExp(pathParam), encodeURI(params[pathParam.substring(1)])), path) + (0, exports.getQueryString)(queryString, queryParams));
    };
};
 ...
 */
const createRouter = ({ routeMatcher, convertMatch, routeConfig, toUrl, rootPath, plainHash = false, queryString = emptyQueryString, wdw = window }) => {
    if (!routeMatcher) {
 ...
 */
const RouteChangeEffect = ({ update, Effects, isRouteChanged = (state) => state.route.changed, routeChangedPatch = { route: { changed: false } } }) => {
...
exports.RouteChangeEffect = RouteChangeEffect;
// ----- Mithril
/**
 ...
 */
const createMithrilRouter = ({ m, routeConfig, rootPath, plainHash = false, wdw = window }) => {
    if (!m) {
        throw 'm is required';
    }
    if (!routeConfig) {
