...
<a name="accept_computed_services"></a>
### [Accept / Computed / Services](#accept_computed_services)

As you saw in the [Acceptor](#acceptor) section, SAM as a concept of accepting or rejecting
proposals. For this to work, the proposals (or _patches_ in Meiosis) must be in a format that
we can inspect.

This is not the case for function patches, and perhaps other forms of patches that you may
like to use. How can we overcome this limitation?

In Meiosis, we can use a separate stream in the same way as _computed_. Let's call this stream
_accept_. Functions will run on the _accept_ stream **before** the _computed_ functions. Thus,
these functions will have the first say in whether the current state is acceptable. We can use
this as the equivalent of SAM's acceptor concept.

The difference is that instead of being functions of (state, proposal), our _accept_ functions
are just functions of state. They return patches that optionally alter the state according to
whether we accept, reject, or modify the current state.

For example, whereas before we had this acceptor function:
...
Now, we have this accept function:
...
We can use this in the same way as the `computed` function we saw in
[Services and Computed Properties](services.html#using_computed_and_services). Our Meiosis
pattern setup is as follows:
...
@flems code/sam-pattern/accept-computed-services.js,app.html,public/css/spectre.css react,react-dom,flyd,patchinko 700 60

By setting up `accept` as a function of state, we no longer need to be able to interpret patches.
We have the flexibility of using patches in the form that we want, such as function patches.
